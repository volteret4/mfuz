...
from tools.ics_manager import ICSManager
...
class MuspyArtistModule(BaseModule):
    def __init__(self,
        ...
        self.logger.info("TwitterManager inicializado con credenciales:")
        ...
        self.ics_manager = ICSManager(self, PROJECT_ROOT)
...
# Métodos de contexto de tabla:
  ...
  # Modificación para muspy_releases_module.py
    ...
    def _build_context_menu(self, menu, info, table_name):
        ...
        # Add ICS export option for release tables at the top
        if table_name in ["tableWidget_muspy_results", "releases_table", "tableWidget_releases"]:
            ...
            if menu.actions():  # Add separator after ICS options
                menu.addSeparator()
    ...
    def _add_ics_export_option(self, menu, table_name):
        """
        Add ICS export option to context menu for release tables

        Args:
            menu (QMenu): Menu to add the option to
            table_name (str): Name of the table
        """
        # Check if ICS manager is available
        if not hasattr(self, 'ics_manager') or not self.ics_manager.is_available():
            return

        # Only add for release tables
        if table_name not in ["tableWidget_muspy_results", "releases_table", "tableWidget_releases"]:
            return

        # Get the table widget
        table = None
        if table_name == "tableWidget_muspy_results":
            # Find the muspy results page
            ...
            if stack_widget:
                for i in range(stack_widget.count()):
                    ...
                    if widget.objectName() == "muspy_results_widget":
                        table = widget.findChild(QTableWidget, "tableWidget_muspy_results")
                        break
        else:
            table = self.findChild(QTableWidget, table_name)
        ...
        # Add ICS export actions
        export_selected_action = QAction("📅 Exportar seleccionados a calendario ICS", self)
        export_selected_action.triggered.connect(
            lambda: self.ics_manager.create_ics_from_selected_table_rows(table)
        )
        menu.addAction(export_selected_action)

        export_all_action = QAction("📅 Exportar todos a calendario ICS", self)
        export_all_action.triggered.connect(
            lambda: self._export_all_releases_to_ics(table)
        )
        menu.addAction(export_all_action)

    def _export_all_releases_to_ics(self, table):
        """
        Export all releases from table to ICS

        Args:
            table (QTableWidget): Table with releases
        """
        if not table:
            return

        # Extract all releases from table
        all_releases = []
        ...
        if all_releases:
            self.ics_manager.create_ics_from_releases(all_releases, table)
        else:
            QMessageBox.warning(
                self,
                "Sin datos",
                "No se pudieron extraer datos válidos de la tabla."
            )
